...
# Download agent binary from GitHub Releases (or PROOFPULSE_BINARY_URL)
echo "Downloading agent from $BINARY_URL ..."
if ! curl -sSLf -o "${INSTALL_DIR}/proofpulse-agent" "$BINARY_URL"; then
  echo "Error: could not download binary. Ensure a release exists with asset proofpulse-agent-linux-amd64."
  exit 1
fi
chmod +x "${INSTALL_DIR}/proofpulse-agent"
